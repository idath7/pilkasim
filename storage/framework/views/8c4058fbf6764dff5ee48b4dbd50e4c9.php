...
<link href="<?php echo e(asset('Assets/vendor/quill.snow.css')); ?>" rel="stylesheet">
<style>
    .header-flex {
        display: flex;
    ...
    <?php $__currentLoopData = $candidates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $candidate): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="card candidate-card" style="position: relative;">
            ...
            <div class="candidate-info">
                <div style="font-weight: 700; font-size: 0.95rem;"><?php echo e($candidate->name); ?></div>
                <div style="color: var(--text-muted); font-size: 0.875rem; margin-bottom: 0.5rem;"><?php echo e($candidate->class_name); ?> | <?php echo e($candidate->organization); ?></div>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: auto;">
                    <span class="badge" style="background: var(--primary); color: white; padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.75rem;"><?php echo e($candidate->votes); ?> Suara</span>
                    
                    <div class="candidate-actions" style="display: flex; gap: 0.5rem;">
                        <button onclick="openEditModal(<?php echo e($candidate->id); ?>, '<?php echo e(addslashes($candidate->name)); ?>', '<?php echo e(addslashes($candidate->class_name)); ?>', '<?php echo e(addslashes($candidate->organization)); ?>', `<?php echo e(base64_encode($candidate->vision)); ?>`, `<?php echo e(base64_encode($candidate->mission)); ?>`)" class="btn" style="padding: 0; font-size: 0.875rem; background-color: transparent; color: var(--primary); border: 1px solid transparent; transition: all 0.2s; border-radius: 6px; display: flex; justify-content: center; align-items: center; width: 32px; height: 32px;" title="Edit" onmouseover="this.style.backgroundColor='#e0e7ff'" onmouseout="this.style.backgroundColor='transparent'"><i class="fa-solid fa-pen"></i></button>
                        
                        <form id="delete-form-<?php echo e($candidate->id); ?>" action="<?php echo e(route('admin.candidates.destroy', $candidate->id)); ?>" method="POST" style="margin: 0;">
                            <?php echo csrf_field(); ?>
                            <button type="button" onclick="confirmDelete(<?php echo e($candidate->id); ?>, '<?php echo e(addslashes($candidate->name)); ?>')" class="btn" style="padding: 0; font-size: 0.875rem; background-color: transparent; color: #ef4444; border: 1px solid transparent; transition: all 0.2s; border-radius: 6px; display: flex; justify-content: center; align-items: center; width: 32px; height: 32px;" title="Hapus" onmouseover="this.style.backgroundColor='#fee2e2'" onmouseout="this.style.backgroundColor='transparent'"><i class="fa-solid fa-trash"></i></button>
                        </form>
                    </div>
                </div>
            </div>
...
<div id="addCandidateModal" class="modal">
    <div class="modal-content" style="max-width: 800px;">
        ...
        <form action="<?php echo e(route('admin.candidates.store')); ?>" method="POST" enctype="multipart/form-data" id="candidateForm">
            <?php echo csrf_field(); ?>
                    ...
                    <div class="form-group" style="background: #f9fafb; padding: 1rem; border-radius: var(--radius); margin-bottom: 1rem; border: 1px solid var(--border);">
                        ...
                        <div class="custom-select-wrapper" style="position: relative; margin-top: 0.5rem;">
                            ...
                            <div class="custom-options-container" id="custom-options" style="display: none; position: absolute; top: 100%; left: 0; right: 0; background: white; border: 1px solid var(--border); border-radius: var(--radius); margin-top: 0.25rem; box-shadow: var(--shadow-lg); z-index: 100; max-height: 250px; overflow-y: auto;">
                                ...
                                <div class="custom-option voter-option" data-name="<?php echo e(strtolower($voter->name)); ?>" onclick="selectVoter('<?php echo e(addslashes($voter->name)); ?>|<?php echo e(addslashes($voter->class_name)); ?>', '<?php echo e(addslashes($voter->name)); ?> (<?php echo e(addslashes($voter->class_name)); ?>)', this)" style="padding: 0.75rem; cursor: pointer; border-bottom: 1px solid #f3f4f6; display: flex; align-items: center; justify-content: space-between; transition: all 0.2s;">
                                    <span><?php echo e($voter->name); ?> <small style="color: var(--text-muted);">(<?php echo e($voter->class_name); ?>)</small></span>
...
<script src="<?php echo e(asset('Assets/vendor/quill.js')); ?>"></script>
<script>
    var toolbarOptions = [
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\1Laravel\pilkasim\resources\views\admin\candidates.blade.php ENDPATH**/ ?>