    ...
    <div style="display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
        <form method="GET" id="filterForm" style="margin: 0; display: flex; align-items: center; gap: 1rem; flex-wrap: wrap;">
            ...
            <div style="position: relative;">
                ...
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama, NIS, username..." class="form-control" style="padding: 0.4rem 1rem 0.4rem 2.25rem; width: 250px; font-size: 0.85rem; border-radius: 20px; border: 1px solid var(--border);">
            </div>
...
<div class="page-container animate-fade-in" style="animation-delay: 0.1s;">
    ...
    <div id="floatingActionSidebar" style="position: fixed; top: 50%; left: -300px; transform: translateY(-50%); width: 280px; background: #fff; padding: 1.5rem; border-radius: 0 12px 12px 0; border: 1px solid var(--border); border-left: none; box-shadow: 5px 0 25px rgba(0,0,0,0.1); z-index: 100; transition: left 0.3s cubic-bezier(0.4, 0, 0.2, 1);">
        ...
        <form action="{{ route('admin.voters.reset_votes') }}" method="POST" onsubmit="confirmAction(event, 'Hasil perolehan suara akan dikosongkan dan status memilih siswa akan direset. Anda yakin?');" style="margin: 0;">
            @csrf
        ...
        <form action="{{ route('admin.voters.reset_all') }}" method="POST" onsubmit="confirmAction(event, 'Peringatan: Seluruh data pemilih akan dihapus dari database! Anda yakin?');" style="margin: 0;">
            @csrf
    ...
    <div class="page-content" style="padding: 1.5rem; overflow-x: auto;">
        ...
        <thead>
            <tr id="defaultHeaders">
                ...
                <th><a href="?search={{ $search }}&per_page={{ $perPage }}&sort=nis&direction={{ $sort == 'nis' && $direction == 'asc' ? 'desc' : 'asc' }}" style="color: inherit; text-decoration: none;">NIS {!! $sort == 'nis' ? ($direction == 'asc' ? '↑' : '↓') : '' !!}</a></th>
                ...
                <th><a href="?search={{ $search }}&per_page={{ $perPage }}&sort=class_name&direction={{ $sort == 'class_name' && $direction == 'asc' ? 'desc' : 'asc' }}" style="color: inherit; text-decoration: none;">Kelas {!! $sort == 'class_name' ? ($direction == 'asc' ? '↑' : '↓') : '' !!}</a></th>
